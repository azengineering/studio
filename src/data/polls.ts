...
export async function getPollForEdit(pollId: string): Promise<Poll | null> {
    ...
    data.questions.forEach((q: any) => q.options.sort((a: any,b: any) => a.option_order - b.option_order));
    return data;
}
...
export async function upsertPoll(poll: Omit<Poll, 'created_at'>): Promise<Poll> {
    const { data, error } = await supabaseAdmin.rpc('upsert_poll_with_children', { poll_data: poll });
    if (error) {
    ...
    if (!newPoll) {
        throw new Error("Failed to retrieve poll after upserting.");
    }
    return newPoll;
}

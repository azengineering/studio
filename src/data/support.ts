...
export async function createSupportTicket(data: Omit<SupportTicket, 'id' | 'status' | 'created_at' | 'resolved_at' | 'admin_notes'>): Promise<void> {
    const stmt = db.prepare(`
        INSERT INTO support_tickets (id, user_id, user_name, user_email, subject, message, status, created_at)
        VALUES (@id, @user_id, @user_name, @user_email, @subject, @message, 'open', @created_at)
    `);
...
export async function updateTicketStatus(ticketId: string, status: TicketStatus, adminNotes: string | null): Promise<void> {
    const stmt = db.prepare(`
        ...
        SET status = @status, admin_notes = @admin_notes, resolved_at = @resolved_at
        WHERE id = @id
    `);
